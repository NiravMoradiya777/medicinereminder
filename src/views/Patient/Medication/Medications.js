...
export default function Medications() {
    ...
    const pageType = location.pathname.split("/").length > 3 ? "doctor" : "patient";
    ...
    const PageIntroduction = () => {
        if (!isPageIntroductionCalled && localStorage.getItem("switchState") === "true") {
            const text = "This is a Manage Medication page, where you can manage your Medication list";
    ...
    const getMedicationData = () => {
        ...
        // axios.get(process.env.REACT_APP_API_BASE_URL+'medicinereminder/patient/medicines/getMedicinesByUserId', {params: {userId: JSON.parse(localStorage.getItem("user")).userId}})
        axios.get(process.env.REACT_APP_API_BASE_URL + 'medicinereminder/patient/medicines/getMedicinesByUserId', { params: { userId: pageType == "patient" ? JSON.parse(localStorage.getItem("user")).userId : location.pathname.split("/")['3'] } })
            .then((response) => {
    ...
    const handleUpdate = () => {
        ...
        axios.put(process.env.REACT_APP_API_BASE_URL + 'medicinereminder/patient/medicines/updateMedicine', obj).then((response) => {
    ...
    const submitForm = () => {
        ...
        let obj = {
            ...
            end_date: edate.format("YYYY-MM-DD"),
            time_to_take: time.format("HH:mm"),
            userId:  pageType == "patient" ? JSON.parse(localStorage.getItem("user")).userId : location.pathname.split("/")['3'] 
        }

        axios.post(process.env.REACT_APP_API_BASE_URL + 'medicinereminder/patient/medicines/addMedicine', obj).then((response) => {
            // medicationData.push(obj);
            getMedicationData();
            setName(''); setDosage(''); setFrequency(1); setStartDate(dayjs()); setEndDate(dayjs()); setTime(dayjs());
            setValue("1");
        }).catch((error) => {
            alert('An error occured!');
            console.log(error);
    ...
    const handleDelete = (id) => {
        if (window.confirm("Are you sure you want to delete?") == true) {
            axios.delete(process.env.REACT_APP_API_BASE_URL + 'medicinereminder/patient/medicines/deleteMedicine', { params: { medicineId: id } }).then((response) => {
                ...
                <TabPanel value="1">
                    ...
                    <TableContainer component={Paper}>
                        <Table sx={{ minWidth: 650 }} aria-label="simple table">
                            ...
                            <TableBody>
                                {medicationData.length === 0 && (<TableRow><TableCell colSpan={6}>no records found</TableCell></TableRow>)}
